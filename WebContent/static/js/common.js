...
/**
*打开Jquery EasyUi的弹出窗口(适用于Jquery Easy Ui)
*
*@paramdialogId将要打开窗口的id
*/
...
function getSingleSelectRow(dataTableId) {
    ...
    var num = rows.length;
    if (num < 1) {
        $.messager.alert('提示消息', '请选择你要操作的记录!', 'info');
    }else  if (num == 1) {
        return rows[0];
    } else {
        $.messager.alert('提示消息', '只允许编辑单条记录!', 'info');
        return null;
    }
}
...
function deleteNoteById(dataTableId, requestURL) {
    var rows = $('#' + dataTableId).datagrid('getSelections');
    var num = rows.length;
    var ids = new Array();
    if (num < 1) {
        ...
        $.messager.confirm('确认', '确定要删除'+num+'条记录吗？', function (r) {
            if (r) {
                for (var i = 0; i < num; i++) {
                     ids .push(rows[i].id);
                }
                $.post(requestURL, { "ids": ids }, function (data,status) {
                    if (status=="success") {
...
/**
*加载页面片段到页面
*@parameter requestURL 片段url
*@parameter data    必须的数据
*@parameter targetId  目标id
**/

function loadPagePart(requestURL,data,targetId){
	$('#'+targetId).load(requestURL,data);
}